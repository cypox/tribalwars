...
function bb_player($var) {
        ...
        $r1 = $db->query("SELECT * FROM users WHERE username='$var'");
        $n1 = $db->numrows($r1);
        if($n1=="1") {
            if(isset($_GET['village'])) { $villageid='village='.$_GET['village'].'&'; } else { $villageid=""; }
            $result = $db->query("SELECT * FROM users WHERE username='$var'");
...
function bb_ally($var) {
        ...
        $r1 = $db->query("SELECT * FROM ally WHERE short='$var'");
        $n1 = $db->numrows($r1);
        if($n1=="1") {
            if(isset($_GET['village'])) { $villageid='village='.$_GET['village'].'&'; } else { $villageid=""; }
...
{
        global $db;
        $r1 = $db->query("SELECT * FROM villages WHERE x='$var[1]' AND y='$var[2]'");
        $n1 = $db->numrows($r1);
        if($n1=="1") {
            if(isset($_GET['village'])) { $villageid='village='.$_GET['village'].'&'; } else { $villageid=""; }
            $result = $db->query("SELECT * FROM villages WHERE x='$var[1]' AND y='$var[2]'");
            while($row = mysqli_fetch_array($result))
            {
