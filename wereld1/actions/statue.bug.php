...
while($row = $db->fetch($result))
{
	...
	while($row1 = $db->fetch($result1))
	{
		if($row1['unit_knight'] > 0)
		{
			...
			$pala_doing = "%p is stationed in %v.";
			if($knightname == "Paladino")
			{
				$pala_doing = str_replace("%p", "Your paladin", $pala_doing);
			}
			else
			{
			...
			{
				$pala_doing = str_replace("%v", "this village", $pala_doing);
			}
		}
	}
	...
	while($row = $db->fetch($result))
	{
		...
		foreach($cl_units->dbname as $key=>$value)
		{
			if($value == 'unit_knight' && $mov_units[$mov_count] > 0)
			{
				...
				if($row['type'] == "attack")
				{
					$pala_doing = "%p is attacking %v.";
				}
				elseif($row['type'] == "support")
				{
					$pala_doing = "%p is supporting %v.";
				}
				else
				{
					$pala_doing = "%p is returning from %v.";
					$gramm_mode = true;
				}
				if($knightname == "Paladino")
				{
				...
				{
					$pala_doing = str_replace("%v", "this village");
				}
...
if ($show_build) {
	...
	if (isset($_GET['action']) && $_GET['action']=="train") {
		...
		switch($check) {
		    case "not_tec":
		    $error = "Unit not yet researched";
		    break;
		    
			case "build_ah":
		    $error = "Academy has to be built";
		    break;
		    
		    case "not_needed":
		    $error ="Building requirements needed are not fulfilled";
		    break;
		    ...
		    case "not_enough_bh":
		    $error = "Not enough resources available or population limit reached!";
			break;


	 		case "to_many_bh":
			$error = "Too much population";
			break;

			case "pala_exists":
			$error = "You already have a Paladin!";
			break;
		}
	}
	...
	if(isset($_GET['action']) && $_GET['action'] == "deploy" && $pala_moveable)
	{
		...
		$pala_time = time() + (($cl_units->get_time(1, 'unit_knight') / 2) / $config['speed']);
	...
	if($mode == "inventory")
	{
		...
		$row = $db->fetch($result);
		$items_found = 0;
		foreach($knight_items->name as $key){
			if($row[$key] == "false")	{
				$row[$key] = false;
			}else{
				$row[$key] = true;
...
$tpl->assign("teste", count($knight_items->name));
?>