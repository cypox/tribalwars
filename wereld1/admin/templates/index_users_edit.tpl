...
$rs_result = mysqli_query($db->get_connection(), $sql); 
...
$select = mysqli_query($db->get_connection(), "SELECT time,ip,id FROM logins WHERE userid = '$id' ORDER BY time ASC LIMIT $start_from, 20");
...
<table class="vis">
<tr><th width="120">Mail: </th><td>{php} $username = urlencode($this->_tpl_vars['username']); $select_mail = mysql_fetch_Array(mysqli_query($db->get_connection(), "SELECT email FROM login.users WHERE username = '$username'")); echo $select_mail[0]; {/php} </td></tr>
<tr><th width="120">IP Inregistrare: </th><td>{php} $username = urlencode($this->_tpl_vars['username']); $select_ipreg = mysql_fetch_Array(mysqli_query($db->get_connection(), "SELECT ip_inregistrare FROM users WHERE username = '$username'")); echo $select_ipreg[0]; {/php} </td></tr>
</table>
