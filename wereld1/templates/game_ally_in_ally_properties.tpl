	...
	<tr>
		...
		{if $user.ally_found == '1'}
			<form action="game.php?village={$village.id}&amp;screen=ally&amp;mode=properties&amp;action=change&amp;h={$hkey}" method="post">
				<table class="vis" width="100%" style="border:1px solid #804000; border-bottom:none;">
					...
					<tr><td width="100">Tag:</td><td><input type="text" name="tag" maxlength="6" value="{$ally.short}" /><br /><b>maximum 6 characters</b></td></tr>
					<tr><td width="100">Homepage:</td><td><input type="text" name="homepage" maxlength="128" size="25"  value="{$ally.homepage}" /></td></tr>
					<tr><td width="100">IRC channel:</td><td><input type="text" name="irc-channel" maxlength="128" size="25"  value="{$ally.irc}" /></td></tr>
					<tr><th colspan="2"><div align="right"><input type="submit" value="Save" class="button" /></div></th></tr>
				</table>
			</form>
			<table class="vis" width="100%" style="border:1px solid #804000;">
				<tr><th>Disband tribe</th></tr>
				<tr><td><a href="javascript:ask('Do you really want to disband your tribe?', 'game.php?village={$village.id}&amp;screen=ally&amp;mode=properties&amp;action=close&amp;h={$hkey}')">&raquo; disband</a></td></tr>
			</table>
		{/if}
		</td>
		<td valign="top" width="50%">
		{if $user.ally_diplomacy == '1'}
			...
			<form action="game.php?village={$village.id}&amp;screen=ally&amp;mode=properties&amp;action=change_desc&amp;h={$hkey}" method="post" name="edit_profile">
				<table class="vis" width="100%" style="border:1px solid #804000; border-bottom:none;">
					<tr><th colspan="2">Tribe description <span style="float:right;"><a id="edit_link" href="javascript:bbEdit()" style="display:none"><span class="button">Edit</span></a><a id="edit_link_close" href="javascript:bbEdit_close()" style="display:none"><span class="button">Cancel</span></a></span></th></tr>
			...
			<form action="game.php?village={$village.id}&amp;screen=ally&amp;mode=properties&amp;action=ally_image&amp;h={$hkey}" enctype="multipart/form-data" method="post">
				<table class="vis" width="100%" style="border:1px solid #804000; border-bottom:none;">
					<tr><th>Coat of arms:</th></tr>
					<tr>
						<td align="center">
							{if !empty($ally.image)}
								<img src="graphic/ally/{$ally.image}" alt="Coat of arms" /><br />
								<input name="del_image" type="checkbox" /> Delete coat of arms<br />
							{/if}
