...
if ($conf['admin_key'] == 'actions_massiv_keyaaassd') {
	...
	if ($_GET['akcja'] == 'dodaj_teama' and isset($_POST)) {
		if (strlen($_POST['tm_gracz']) < 3) {
			$err = 'Nazwa gracza jest za kr�tka!';
			} else {
			...
			if (strlen($_POST['tm_opis']) > 50) {
				...
				$db->query("INSERT INTO team(gracz,opis) VALUES('".$_POST['tm_gracz']."','".$_POST['tm_opis']."')");
	...
	$admin = $_GET['admin'];
	$update = $db->query("UPDATE gracze SET admin = '$admin' WHERE id = '".$_GET['oid']."'");
	header ("Location: admin.php?screen=uzytkownicy");
}

	if ($_GET['akcja'] == 'aktywuj_wszystkim') {

	$update = $db->query("UPDATE gracze SET aktywowano = '1'");
	header ("Location: admin.php?screen=uzytkownicy");
}
	...
	if ($_GET['akcja'] == 'usun_usera' and isset($_GET['oid'])) {
		...
		$counts = sql("SELECT COUNT(id) FROM  `gracze` WHERE `id` = '".$_GET['oid']."'",'array');
		if ($counts > 0) {
			$db->query("DELETE FROM `gracze` WHERE `id` = '".$_GET['oid']."'");
			}
		header('LOCATION: admin.php?screen=uzytkownicy');
		exit;
		}
		
	$query['big_arr'] = $db->query("SELECT * FROM `gracze` ORDER BY `id` LIMIT $RA_Limit , 45");
	while ($us_info = $db->fetch($query['big_arr'])) {
	...
	if ($_GET['akcja'] == 'usun_teama' and isset($_GET['oid'])) {
		...
		if ($counts > 0) {
			$db->query("DELETE FROM `team` WHERE `id` = '".$_GET['oid']."'");
			}
		header('LOCATION: admin.php?screen=uzytkownicy');
		exit;
		}
		
	$query['big_arr'] = $db->query("SELECT * FROM `team`");
