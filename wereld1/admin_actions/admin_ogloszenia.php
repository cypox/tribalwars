...
if ($conf['admin_key'] == 'actions_massiv_keyaaassd') {
	if ($_GET['akcja'] == 'dodaj_ogloszenie' and isset($_POST)) {
		if (strlen($_POST['ogl_val']) < 5) {
			$err = 'Og�oszenie jest za kr�tkie (min. 5 znak�w)';
			} else {
			if (strlen($_POST['ogl_val']) > 5000) {
				$_POST['ogl_val'] = urlencode($_POST['ogl_val']);
				$err = 'Og�oszenie jest za d�ugie (maks. 5000 znak�w)'; 

				} else {
			if (strlen($_POST['ogl_nazwa']) > 6) {
				$_POST['ogl_nazwa'] = urlencode($_POST['ogl_nazwa']);
				$err = 'W nazwie musi by� najwy�ej 6 znak�w!';  
			} else {
				$db->query("INSERT INTO ogloszenia(data,text,typ,nazwa) VALUES('".date("U")."','".$_POST['ogl_val']."','".$_POST['ogl_typ']."','".$_POST['ogl_nazwa']."')");
	...
	if ($_GET['akcja'] == 'usun_ogloszenie' and isset($_GET['oid'])) {
		...
		if ($counts > 0) {
			$db->query("DELETE FROM `ogloszenia` WHERE `id` = '".$_GET['oid']."'");
			}
		header('LOCATION: admin.php?screen=ogloszenia');
		exit;
		}
		
	$query['big_arr'] = $db->query("SELECT * FROM `ogloszenia` ORDER BY data DESC LIMIT 30");
	while ($og_info = $db->fetch($query['big_arr'])) {
