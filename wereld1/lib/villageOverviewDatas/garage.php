...
$i = 0;
$now = time();
$overdue = $db->query("SELECT `id` FROM `recruit` WHERE `villageid`='".$village['id']."' AND `building`='garage' AND `time_finished`<='".$now."'");
...
$res = $db->query("SELECT `unit`,`num_unit`,`num_finished`,`time_finished` FROM `recruit` WHERE `villageid`='".$village['id']."' AND `building`='garage' ORDER BY `time_finished`");
