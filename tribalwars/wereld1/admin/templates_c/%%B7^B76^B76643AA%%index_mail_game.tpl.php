<?php /* Smarty version 2.6.26, created on 2013-07-16 18:33:31
         compiled from index_mail_game.tpl */ ?>
<?php 
$timp=time();
...
if($_GET["action"] == "view") {
   ...
   $exec_sql2 = mysqli_query($db->get_connection(), $sql3) or die(mysqli_error($db->get_connection())); 
   while ($array2 = mysqli_fetch_array($exec_sql2)) {
   ...
   $exec_sql6 = mysqli_query($db->get_connection(), $sql7) or die(mysqli_error($db->get_connection()));
   $numar=-1;
   ...
   while ($array2 = mysqli_fetch_array($exec_sql6)) {
    ...
    if ($no==0) {
      ...
      $exec_sql2 = mysqli_query($db->get_connection(), $sql3) or die(mysqli_error($db->get_connection())); 
      $nr_user1 = mysqli_num_rows($exec_sql2);
      if ($nr_user1==0) { $mesaj["nume_from_display"][] = "negasit"; }
      while ($array = mysqli_fetch_array($exec_sql2)) {
   ...
   $nr_mesaj1 = mysqli_num_rows($exec_sql6);
   ...
   $nr_mesaj2 = mysqli_num_rows($exec_sql6);
   ...
   $nr_mesaj3 = mysqli_num_rows($exec_sql6);
...
<form action="?screen=mail_game&action=view" method="post">
        Jucatorul 1:<input name="from" type="text" value="<?php echo stripslashes(urldecode($user_from)); ?>" />
           

        Jucatorul 2:<input name="to" type="text" value="<?php echo stripslashes(urldecode($user_to)); ?>"/><br>
Rezultatul limita <input name="limit" type="text" size="2" value="<?php echo stripslashes(urldecode($limit)); ?>"/>
Mesaje mai noi de: <input name="compare" type="text" size="2" value="<?php echo stripslashes(urldecode($compare)); ?>"/> ore
...
if ($numar_ordine<=$limit OR $limit==0) { 
 ?>
<b><?php echo $numar_ordine; ?>) Luate de la: <?php echo $mesaj["tip_mesaj"][$key]; ?> ID: <?php echo $mesaj["id_mesaj"][$key]; ?></b>
<table class="vis" width="100%"> 
		<tr><th >Subject:</th><td > 
		
		<?php echo $mesaj["subject_mesaj"][$key]; ?>
		
		</td><th>Trimis:</td><td><?php echo $mesaj["time_mesaj"][$key] ?></th></tr> 
		<tr><th>Expeditor | ID:</td><td><?php echo $mesaj["nume_from_display"][$key],' | ',$mesaj["id_from_display"][$key]; ?>
		</th><th>Destinatar | ID:</th><td><?php echo $mesaj["nume_to_display"][$key],' | ',$mesaj["id_to_display"][$key]; ?></th></tr> 
...
<?php echo stripslashes($mesaj["text_mesaj"][$key]); ?>	</td></tr> 
		</table> <br>
