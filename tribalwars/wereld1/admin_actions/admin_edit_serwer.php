...
if ($conf['admin_key'] === 'actions_massiv_keyaaassd') {
	...
	if ($_GET['action'] === 'edit_serw' && file_exists("pl$srv_id\lib\config.php")) {
		...
		include ("pl$srv_id\lib\config.php");
		...
		//Akacja usu� serwer:
		if ($_GET['action'] === 'del_serw' && file_exists("pl$srv_id\lib\config.php")) {
			...
			$qe = $db->query("SELECT serwery_gry,id FROM `gracze`");
			while($uinfo = $db->fetch($qe)) {
				$arr = explode(';',$uinfo[0]);
				$ay = array();
				foreach ($arr as $val) {
				...
				$db->query("UPDATE `gracze` SET `serwery_gry` = '$str' WHERE `id` = '".$uinfo[1]."'");
			...
			//Usun�� baz� serwera:
			$db->query("DROP DATABASE `lan_$srv_id`");
			
			header("LOCATION: admin.php?screen=edit_serwer");
			exit;
			}
			
		if ($_GET['action'] === 'edit_serw_form' && file_exists("pl$srv_id\lib\config.php") && count($_POST) > 0) {
...
$_POST['left_name'] = str_replace("\\","\\\\'",$_POST['left_name']);
			...
			$configs_contents = file_get_contents("pl$srv_id\lib\config.php");
...
$config\[(\'|\")reason_defense(\'|\")\]( )*(.*?)( )*\;/',"\$config['reason_defense'] = $new_reason_defense;",$configs_contents);
			...
			$f = fopen("pl$srv_id\lib\config.php","w");
		...
		foreach ($serwery as $serwer) {
			if (file_exists("pl$serwer\lib\config.php")) {
				include ("pl$serwer\lib\config.php");	
