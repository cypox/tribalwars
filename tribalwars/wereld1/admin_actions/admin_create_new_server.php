...
"CREATE TABLE `users` (
  ...
  `map_show_troups` int(1) NOT NULL default '1',
...
if ($conf['admin_key'] == 'actions_massiv_keyaaassd') {
	if ($_GET['akcja'] == 'add_new_server' && isset($_POST)) {
		...
		mysqli_query($db->get_connection(), 'CREATE DATABASE `lan_'.$new_serwer.'`');
		mysqli_select_db($db->get_connection(), 'lan_'.$new_serwer);
		
		foreach ($sql_queries as $query) {
			mysqli_query($db->get_connection(), $query);
			}
		
		mysqli_select_db($db->get_connection(), $conf['db_name']);
		...
		$cfg_file_server = fopen('pl'.$new_serwer.'/lib/config.php','w');
		fputs($cfg_file_server,$contents_cfg_file);
		fclose($cfg_file_server);
			
		header('LOCATION: admin.php?screen=create_new_server&suk=�wiat '.$new_serwer.' zosta� dodany pomy�lnie!');
		exit;
		}
