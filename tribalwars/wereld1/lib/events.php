...
function do_movement_attack($row,$event_id,$event_time,$var=""){
	...
	if($config['statue_style'] > 1)
	{
		// Off ist einfach
		if($att['unit_knight'] > 0)
		{
			$result = $db->query("SELECT * FROM `knight_items` WHERE `uid` = '".$row['from_userid']."'");
			$k_items = $db->fetch($result);
			if(isset($knight_items->name[$k_items['chosen']]))
			{
				$others['att_knight_items'][] = $k_items['chosen'];
			}
		}
		// Deff ist bissel schwieriger
		if($def['unit_knight'] > 0)
		{
			$result1 = $db->query("SELECT * FROM `unit_place` WHERE `villages_to_id` = '".$row['to_village']."'");
			while($row1 = $db->fetch($result1))
			{
				$result2 = $db->query("SELECT * FROM `villages` WHERE `id` = '".$row1['villages_from_id']."'");
				while($row2 = $db->fetch($result2))
				{
					$result = $db->query("SELECT * FROM `knight_items` WHERE `uid` = '".$row2['userid']."'");
					$k_items = $db->fetch($result);
					if(isset($knight_items->name[$k_items['chosen']]))
					{
						$others['def_knight_items'][] = $k_items['chosen'];
					}
				}
			}
		}
	...
	if($simulate['att']['unit_snob']-$simulate['att_lose']['unit_snob'] != 0){
		if(in_array("snob", $simulate['others']['att_knight_items'])) {
		    $config->set('agreement_min', $knight_items->special['snob']);
		}
	...
	foreach($abzug as $id=>$arr){
		...
		if($id != $row['to_village']){
			...
			if($all_units_died && !$config['attack_visit'])
...
function check_events(){
	...
	// Pala verschieben
	$result = $db->query("SELECT * FROM `events` WHERE `event_type` = 'deploy_pala'");
	while($row = $db->fetch($result))
	{
		if($row['event_time'] <= time())
		{
			$db->query("UPDATE `unit_place` SET `unit_knight` = '1' WHERE (`villages_from_id` = '".$row['villageid']."' AND `villages_to_id` = '".$row['villageid']."')");
			$db->query("DELETE FROM `events` WHERE `id` = '".$row['id']."'");
		}
	}
	...
	if($config['statue_style'] > 1)
	{
		...
		while($row = $db->fetch($result))
		{
			$result1 = $db->query("SELECT SUM(statue) AS statue FROM `villages` WHERE `userid` = '".$row['id']."'");
			$row1 = $db->fetch($result1);
			if($row1['statue'] > 0)
			{
				$result2 = $db->query("SELECT * FROM `knight_items` WHERE `uid` = '".$row['id']."'");
				if($db->numRows($result2) <= 0)
				{
					$db->query("INSERT INTO `knight_items` (`uid`, `progress`, `last_actu`) VALUES ('".$row['id']."', '0', '".(time())."')");
				}
				else {
				$row2 = $db->fetch($result2);
				$not_get = array();
				foreach($knight_items->name as $key=>$name)
				{
					if($row2[$key] == "false")
					{
						$row2[$key] = false;
				...
				if(count($not_get) > 0)
				{
					...
					if($row2['last_actu'] + round((24*60*60)/$config['speed']) <= time())
					{
						$actu = time();
						$progress += 3;
					}
					...
					{
						$progress -= 100;
						$new_item = rand(0, count($not_get) - 1);
						$db->query("UPDATE `knight_items` SET `".$not_get[$new_item]."` = 'true' WHERE `uid` = '".$row['id']."'");
						
						$cl_reports->new_knight_item($row['id'], $not_get[$new_item]);
						
					}
					$db->query("UPDATE `knight_items` SET `progress` = '".$progress."', `last_actu` = '".$actu."' WHERE `uid` = '".$row['id']."'");
				}
				}
			}
		}
...
function check(){
	...
	$f = $db->query("SELECT * FROM `".$config['global_db']."`.`premium_feature`");
	while($a = $db->fetch($f)){
		if(time() > $a['activated_until']){
			$db->query("UPDATE users SET premium_active = '0' WHERE id = '".$a['userid']."'");
		}
	}
