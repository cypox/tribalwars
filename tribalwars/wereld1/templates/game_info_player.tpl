	...
	<tr>
		<td valign="top" width="45%">
			<table class="vis" width="100%" style="border:1px solid #804000;">
				...
				{/if}
				<tr><td colspan="2"><a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=new&amp;player={$info_user.id}">&raquo; Send message</a></td></tr>
			{if $can_invite}
				<tr><td colspan="2"><a href="javascript:ask('Do you want to invite player {$info_user.username} to your tribe?', 'game.php?village={$village.id}&screen=ally&mode=invite&action=invite_id&id={$info_user.id}&h={$hkey}')">&raquo; Invite to tribe</a></td></tr>
			{/if}
			{if $friend_invite && $info_user.id != $user.id}
				<tr><td colspan="2"><a href="game.php?village={$village.id}&screen=friends&amp;action=add_buddy_id&id={$info_user.id}">&raquo; Add as friend</a></td></tr>
			{/if}
			</table><br />
			<table class="vis" width="100%" style="border:1px solid #804000;">
				<tr>
		...
		<td align="right" valign="top" width="55%">
			...
			<table class="vis" width="100%" style="border:1px solid #804000;">
				<tr><th colspan="2">Profile <span style="float:right;">{if $info_user.id == $user.id}<a href="game.php?village={$village.id}&screen=settings&mode=profile"><span class="button">Edit</span></a>{/if}</span></th></tr>
				{if !empty($info_user.image)}
				<tr><td colspan="2" align="center"><img src="{$config.cdn}/graphic/player/{$info_user.image}" title="Coat of arms of: {$info_user.username}" /></td></tr>
				{/if}
				{if $age!=-1}
			...
			<table width="100%" class="vis" style="border:1px solid #804000;">
				...
				<tr>
					<td valign="bottom">
						...
						{if $info_user.id == $user.id}<div style="font-size:7pt; color:#666; margin-top:2px;">&raquo; Next level: {$cl_awards->get_nextStage($dbname,$medalha.id)}</div>{/if}
					</td>
				</tr>
				{/foreach}
			</table><br />
			...
			<table width="100%" class="vis" style="border:1px solid #804000;">
				<tr><th colspan="2">Medals not yet earned</th></tr>
