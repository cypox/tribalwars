...
{php} 
	...
	$res100 = mysqli_query($db->get_connection(), $sql100);
	while($row100 = mysqli_fetch_assoc($res100))
	{
		$time_end = $row100[end_time];
		
	}
	mysqli_free_result($res100);
	...
	$res101 = mysqli_query($db->get_connection(), $sql101);
	while($row101 = mysqli_fetch_assoc($res101))
	{
	...
	$res1 = mysqli_query($db->get_connection(), $sql1);
	$main = ".png";
	$smith = ".png"; 
	...
	$res2 = mysqli_query($db->get_connection(), $sql2);
	...
	$res = mysqli_query($db->get_connection(), $sql);
	echo <<<FA

FA;

	$sql3 = "SELECT * FROM villages WHERE id =\"". $_GET["village"] ."\"";
	$res3 = mysqli_query($db->get_connection(), $sql3);
	...
	$res4 = mysqli_query($db->get_connection(), $sql4);
...
{php}
	if($_GET[stufen] == "yes")
	{
		...
		mysqli_query($db->get_connection(), $sql12);
	}
	if($_GET[stufen] == "no")
	{
		$sql13 = "UPDATE users SET stufen = \"no\" WHERE id = ".$userid."  LIMIT 1;";
		mysqli_query($db->get_connection(), $sql13);
	}
{/php}
{php}
	...
	$res5 = mysqli_query($db->get_connection(), $sql5);
