...
$ranks = array();
$allys = array();
$result = $db->query("SELECT `id`,`killed_units_att_rank`,`username`,`killed_units_att` FROM `users` ORDER BY `killed_units_att_rank` LIMIT ".$start.",".$ranks_per_page."");
while($row = $db->fetch($result)){
    $ranks[$row['id']]['username'] = entparse($row['username']);
    $ranks[$row['id']]['rang'] = $row['killed_units_att_rank'];
    $ranks[$row['id']]['killed_units'] = format_number($row['killed_units_att']);
	$ranks[$row['id']]['mark'] = "";
