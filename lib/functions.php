...
function create_village($userid,$username='',$direction){
	...
	if($userid != "-1"){
 		...
 		$villagename = parse($config['left_name']);
 	}	
...
function format_date($stamp, $show_sek="false"){
	...
	if($today_day == date("d", $stamp)){
		...
		$return = " ".date("d.m", $stamp);
	}
	if($show_sek){
		$return .= " om ".date("G:i:s", $stamp)." ";
	}else{
		$return .= " om ".date("G:i", $stamp)." ";
	}
	return $return;
}
...
function simulate($att,$att_tech,$def,$def_tech,$others){
	...
	foreach($cl_units->get_array("dbname") as $name=>$dbname){
		if($cl_units->get_group($dbname) == "cav"){
			$attPoints_Cav += $cl_units->get_att($dbname,$att_tech[$dbname])*$att[$dbname];
		}elseif($cl_units->get_group($dbname) == "archer"){
			$attPoints_Bow += $cl_units->get_att($dbname,$att_tech[$dbname])*$att[$dbname];
		}else{
			$attPoints_Foot += $cl_units->get_att($dbname,$att_tech[$dbname])*$att[$dbname];
		}
		$attTotal += $att[$dbname];
		$defTotal += $def[$dbname];
		$defPoints_Foot += $cl_units->get_def($dbname,$def_tech[$dbname])*$def[$dbname];
		$defPoints_Cav += $cl_units->get_defCav($dbname,$def_tech[$dbname])*$def[$dbname];
		$defPoints_Bow += $cl_units->get_defCav($dbname,$def_tech[$dbname])*$def[$dbname];
	}
	...
	if($attPoints_real > $defPoints_real){
		...
		if($att['unit_catapult'] > 0){
			...
			$max_build_dmg = (($att['unit_catapult']*$cl_units->get_att("unit_catapult",$att_tech["unit_catapult"]*$others['believe_att_factor']))/($catapultHarm['base']*pow($catapultHarm['factor'], $others['def_building'])));
			if($attTotal <= 0){
				$cata_factor = 1;
			}else{
		...
		if($att['unit_catapult'] > 0){
			...
			$catapultHarm = $config['catapult']['wall'];
			$others['new_building'] = $others['def_building']-round(($att['unit_catapult']*$cl_units->get_att("unit_catapult",$att_tech["unit_catapult"])*$others['believe_att_factor']*$cata_factor)/(2*$catapultHarm['base']*pow($catapultHarm['factor'], $others['def_building'])));
		}else{
			$others['new_building'] = $others['def_building'];
		}
...
function get_movement_message($type,$villagename,$from){
	if($from == "own"){
		switch($type){
			case "attack" :
				return "Ataque à ".$villagename;
			case "support" :
				return "Apoio à ".$villagename;
			case "back" :
				return "Retirada de ".$villagename;
			case "return" :
				return "Retorno de ".$villagename;
			case "cancel" :
				return "Retirada de ".$villagename;
		}
	}else{
...
function UserPermission($permission = '', $uid = ''){
	global $db;
	$permission_rank = $db->fetch($db->query("SELECT * FROM `permissions_ranks` WHERE `rank` = '".UserRank($uid)."' LIMIT 1"));
	if($permission_rank[$permission] == '1'){
		return true;
	}else{
		return false;
	}
...
function UserRank($uid = ''){
	global $db;
	$user = $db->fetch($db->query("SELECT * FROM `users` WHERE `username` = '".$uid."' LIMIT 1"));
...
function inputbb(){
	...
	<div style="position: relative"> 
		<a title="Bold" href="javascript:nothing();" onclick="BBCodes.insert( '[b]', '[/b]');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat 0px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Italic" href="javascript:nothing();" onclick="BBCodes.insert( '[i]', '[/i]');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -20px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Underline" href="javascript:nothing();" onclick="BBCodes.insert( '[u]', '[/u]');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -40px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Stroke" href="javascript:nothing();" onclick="BBCodes.insert( '[s]', '[/s]');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -60px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Quote" href="javascript:nothing();" onclick="BBCodes.insert( '[quote=Autor]', '[/quote]');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -140px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Link" href="javascript:nothing();" onclick="BBCodes.insert( '[url]', '[/url]');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -160px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Spoiler" href="javascript:nothing();" onclick="BBCodes.insert( '[spoiler]', '[/spoiler]');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -260px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Player" href="javascript:nothing();" onclick="BBCodes.insert( '[player]', '[/player]');"> 
														<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -80px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Tribe" href="javascript:nothing();" onclick="BBCodes.insert( '[ally]', '[/ally]');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -100px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Village" href="javascript:nothing();" onclick="BBCodes.insert( '[village]', '[/village]');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -120px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Reports" href="javascript:nothing();" onclick="javascript:toggle_visibility('report_type');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -240px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Marimea textului" href="javascript:nothing();" onclick="javascript:toggle_visibility('bb_sizes');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -220px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Image" href="javascript:nothing();" onclick="BBCodes.insert( '[img]', '[/img]');"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -180px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<a title="Color" href="javascript:nothing();" onclick="javascript:bb_color_picker_toggle();"> 
			<div style="float: left; background:url(http://cdn.infernal-wars.com/graphic/bbcodes/bbcodes.png?1) no-repeat -200px 0px; padding-left: 0px; padding-bottom:0px; margin-right: 4px; width: 20px; height: 20px"></div> 
		</a> 
		<table id="bb_sizes" style="display: none; clear:both;"> 
			<tr> 
				<td> 
					<a href="javascript:nothing();" onclick="BBCodes.insert( '[size=6]', '[/size]');javascript:toggle_visibility('bb_sizes');">&raquo; Very small</a><br /> 
					<a href="javascript:nothing();" onclick="BBCodes.insert( '[size=7]', '[/size]');javascript:toggle_visibility('bb_sizes');">&raquo; Small</a><br /> 
					<a href="javascript:nothing();" onclick="BBCodes.insert( '[size=9]', '[/size]');javascript:toggle_visibility('bb_sizes');">&raquo; Normal</a><br /> 
					<a href="javascript:nothing();" onclick="BBCodes.insert( '[size=12]', '[/size]');javascript:toggle_visibility('bb_sizes');">&raquo; Big</a><br /> 
					<a href="javascript:nothing();" onclick="BBCodes.insert( '[size=20]', '[/size]');javascript:toggle_visibility('bb_sizes');">&raquo; Very big</a><br /> 
				</td> 
			</tr> 
		</table> 
		<table id="report_type" style="display: none; clear:both;"> 
			<tr> 
				<td> 
					<a href="javascript:nothing();" onclick="BBCodes.insert( '[report_display]', '[/report_display]');javascript:toggle_visibility('report_type');">&raquo; Direct show</a><br /> 
					<a href="javascript:nothing();" onclick="BBCodes.insert( '[report]', '[/report]');javascript:toggle_visibility('report_type');">&raquo; Show through window</a><br /> 
				</td> 
			</tr> 
		</table> 
	 	<div id="bb_color_picker" style="display: none; clear:both;"> 
			...
			<div id="bb_color_picker_tones"> 
				<div id="bb_color_picker_10"></div><div id="bb_color_picker_11"></div><div id="bb_color_picker_12"></div>
				<div id="bb_color_picker_13"></div><div id="bb_color_picker_14"></div><div id="bb_color_picker_15"></div>
				<br clear=all /> 
				<div id="bb_color_picker_20"></div><div id="bb_color_picker_21"></div><div id="bb_color_picker_22"></div>
				<div id="bb_color_picker_23"></div><div id="bb_color_picker_24"></div><div id="bb_color_picker_25"></div>
				<br clear=all /> 
				<div id="bb_color_picker_30"></div><div id="bb_color_picker_31"></div><div id="bb_color_picker_32"></div>
				<div id="bb_color_picker_33"></div><div id="bb_color_picker_34"></div><div id="bb_color_picker_35"></div>
				<br clear=all /> 
				<div id="bb_color_picker_40"></div><div id="bb_color_picker_41"></div><div id="bb_color_picker_42"></div>
				<div id="bb_color_picker_43"></div><div id="bb_color_picker_44"></div><div id="bb_color_picker_45"></div>
				<br clear=all /> 
				<div id="bb_color_picker_50"></div><div id="bb_color_picker_51"></div><div id="bb_color_picker_52"></div>
				<div id="bb_color_picker_53"></div><div id="bb_color_picker_54"></div><div id="bb_color_picker_55"></div>
				<br clear=all /> 
			</div> 
