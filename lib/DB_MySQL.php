...
class DB_MySQL{
	...
	function connect($host="",$user="",$pass="",$db=""){
		...
		if(!$this->connection){
			printf('No database connection could be established.<br />Error message: %s', htmlentities(mysqli_connect_error()));
			exit;
		}
		return true;
	}
	...
	function query($query,$show_error=true){
		...
		if($show_error){
			...
			if(!$result){
				printf('Failing SQL: %s<br />Error: %s', htmlentities($query), htmlentities(mysqli_error($this->connection)));
				exit;
			}
		}
		return $result;
